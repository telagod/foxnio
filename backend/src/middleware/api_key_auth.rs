...
impl IntoResponse for ApiKeyAuthError {
    fn into_response(self) -> Response {
        let (status, error_msg) = match self {
            ApiKeyAuthError::MissingApiKey => (StatusCode::UNAUTHORIZED, "Missing API key"),
            ApiKeyAuthError::InvalidApiKey => (StatusCode::UNAUTHORIZED, "Invalid API key"),
...
fn update_last_used_time(db: &sea_orm::DatabaseConnection, api_key_id: uuid::Uuid) {
    ...
    tokio::spawn(async move {
        let now = Utc::now();
        if let Ok(Some(api_key)) = api_keys::Entity::find_by_id(api_key_id).one(&db).await {
