...
/// 拦截遥测请求中间件
pub async fn block_telemetry(req: Request<Body>, next: Next) -> Response<Body> {
    // 检查是否为遥测端点
...
mod tests {
    ...
    fn test_is_telemetry_endpoint() {
        assert!(is_telemetry_endpoint(
            &"https://sentry.io/api/envelope".parse().unwrap()
        ));
        assert!(is_telemetry_endpoint(
            &"https://api.amplitude.com/track".parse().unwrap()
        ));
        assert!(!is_telemetry_endpoint(
