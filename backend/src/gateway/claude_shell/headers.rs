...
pub fn build_headers(api_key: &str, api_version: &str) -> HeaderMap {
    ...
    headers.insert("anthropic-version", HeaderValue::from_str(api_version).unwrap());
...
/// 构建带遥测的请求头
pub fn build_headers_with_telemetry(api_key: &str, api_version: &str, request_id: &str) -> HeaderMap {
    let mut headers = build_headers(api_key, api_version);
    
    // 请求 ID
    headers.insert("x-client-request-id", HeaderValue::from_str(request_id).unwrap());
...
/// 构建 Beta 功能请求头
pub fn build_headers_with_beta(api_key: &str, api_version: &str, beta_features: &[&str]) -> HeaderMap {
    let mut headers = build_headers(api_key, api_version);
...
mod tests {
    ...
    fn test_build_headers_with_beta() {
        let headers = build_headers_with_beta("test-key", "2023-06-01", &["feature-1", "feature-2"]);
