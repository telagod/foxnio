...
impl RedisPool {
    ...
    pub fn new(config: &RedisConfig) -> Result<Self> {
        ...
        let local_cache = LruCache::new(
            NonZeroUsize::new(config.local_cache_size).unwrap()
        );
        
        info!(
            "Redis client created successfully (pool_size={}, local_cache={})",
            config.pool_size,
            config.enable_local_cache
        );
    ...
    pub async fn set(&self, key: &str, value: &str, ttl: Option<Duration>) -> Result<()> {
        ...
        self.stats.record_request(start.elapsed().as_millis() as u64);
        result
    }
