...
pub async fn enable_totp(
    ...
    if claims.is_temp {
        return Err(ApiError(
            ...
            "Cannot enable TOTP with temporary token".into(),
        ));
    }
...
pub async fn confirm_enable_totp(
    ...
    if claims.is_temp {
        return Err(ApiError(
            StatusCode::FORBIDDEN,
            "Cannot confirm TOTP with temporary token".into(),
        ));
    }
    ...
    user_service
        .confirm_enable_totp(user_id, &req.code)
        .await
        .map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
...
pub async fn disable_totp(
    ...
    if claims.is_temp {
        return Err(ApiError(
            StatusCode::FORBIDDEN,
            "Cannot disable TOTP with temporary token".into(),
        ));
    }
...
pub async fn verify_totp(
    ...
    let is_valid = user_service
        .verify_totp(user_id, &req.code)
        .await
        .map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
...
pub async fn regenerate_backup_codes(
    ...
    if claims.is_temp {
        return Err(ApiError(
            StatusCode::FORBIDDEN,
            "Cannot regenerate backup codes with temporary token".into(),
        ));
    }
    ...
    let backup_codes = user_service
        .regenerate_backup_codes(user_id, &req.code)
        .await
        .map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
...
pub async fn totp_login(
    ...
    let (user, token_pair) = user_service
        .login_with_totp(
            &req.temp_token,
            &req.code,
