...
impl RedisRateLimiter {
    ...
    pub async fn check_rate_limit(&self, key: &str) -> Result<RateLimitResult> {
        ...
        if current >= self.config.max_requests {
            // 获取剩余时间
            let ttl: i64 = conn.ttl(&redis_key).await.unwrap_or(self.config.window_seconds as i64);
    ...
    pub async fn check_rate_limit_atomic(&self, key: &str) -> Result<RateLimitResult> {
        ...
        Ok(RateLimitResult {
            ...
            current_count,
            limit,
            reset_after: if ttl > 0 { ttl as u64 } else { self.config.window_seconds },
    ...
    pub async fn check_rate_limit_local(&self, key: &str) -> RateLimitResult {
        ...
        // 统计当前 key 的请求数
        let current_count = counters.iter()
            .filter(|(k, _, _)| k == key)
            .count() as u64;
...
impl DistributedRateLimiter {
    ...
    pub async fn check_user_limit(&self, user_id: &str) -> Result<RateLimitResult> {
        let config = self.user_limits.get(user_id)
            .cloned()
            .unwrap_or_default();
        ...
        if let Some(ref limiter) = self.redis_limiter {
            ...
            Ok(self.check_local_limit(&format!("user:{}", user_id), config).await)
        }
    }
    
    /// 检查 API Key 级别的速率限制
    pub async fn check_api_key_limit(&self, api_key_id: &str) -> Result<RateLimitResult> {
        let config = self.api_key_limits.get(api_key_id)
            .cloned()
            .unwrap_or_default();
        ...
        if let Some(ref limiter) = self.redis_limiter {
            let mut limiter = limiter.clone();
            limiter.config = config;
            limiter.check_rate_limit(&format!("apikey:{}", api_key_id)).await
        } else {
            Ok(self.check_local_limit(&format!("apikey:{}", api_key_id), config).await)
        }
    }
