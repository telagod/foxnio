...
impl ModelRateLimiter {
    ...
    pub async fn check_rate_limit(
                    ...
                    if let Some(rpm) = config.requests_per_minute {
                        if state.request_count >= rpm {
                            ...
                            return Ok(RateLimitStatus {
                                ...
                                remaining_tokens: state.token_count.checked_sub(config.tokens_per_minute.unwrap_or(0)),
                                reset_at: Some(state.window_start + chrono::Duration::seconds(config.duration_secs as i64)),
        ...
        if let Some(ref redis_client) = self.redis {
            return self.check_redis(redis_client, account_id, model, &config).await;
        }
    ...
    pub async fn record_request(
        ...
        // 更新 Redis（如果启用）
        if let Some(ref redis_client) = self.redis {
            self.record_redis(redis_client, account_id, model, tokens, &config).await?;
        }
        
        debug!("Recorded request for account {} model {}", account_id, model);
    ...
    pub async fn get_remaining_time(
        ...
        let status = self.check_rate_limit(account_id, model).await.unwrap_or_else(|_| RateLimitStatus {
            account_id,
            model: model.to_string(),
            is_limited: false,
    ...
    pub async fn reset_limit(&self, account_id: i64, model: &str) -> Result<()> {
        ...
        debug!("Reset rate limit for account {} model {}", account_id, model);
...
mod tests {
    ...
    async fn test_model_specific_config() {
        ...
        model_configs.insert("gpt-4".to_string(), RateLimitConfig {
            requests_per_minute: Some(5),
            tokens_per_minute: None,
            duration_secs: 60,
        });
