...
mod tests {
    ...
    async fn test_redis_connection() {
        ...
        let mut conn = client.get_async_connection().await.expect("Failed to connect");
    ...
    async fn test_redis_ttl_operations() {
        ...
        // SET with EX
        let result: String = conn.set_ex(key, "value", 60).await.expect("Failed to setex");
        assert_eq!(result, "OK");
    ...
    async fn test_redis_hash_operations() {
        ...
        // HSET
        let result: bool = conn.hset(key, "field1", "value1").await.expect("Failed to hset");
        assert!(result);
        ...
        // HSET multiple
        let result: i32 = conn.hset_multiple(key, &[("field2", "value2"), ("field3", "value3")])
            .await
            .expect("Failed to hset multiple");
        assert_eq!(result, 2);
        
        // HGETALL
        let result: std::collections::HashMap<String, String> = conn.hgetall(key).await.expect("Failed to hgetall");
    ...
    async fn test_redis_set_operations() {
        ...
        // SISMEMBER
        let result: bool = conn.sismember(key, "member1").await.expect("Failed to sismember");
        assert!(result);
        
        // SISMEMBER (not exists)
        let result: bool = conn.sismember(key, "member2").await.expect("Failed to sismember");
        assert!(!result);
